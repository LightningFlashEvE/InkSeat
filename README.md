...
| 环节 | 主要职责 | 与下一环节的关系 |
|------|----------|------------------|
| Web 管理端 | 管理设备、设计模板、下发名单、处理并发布图片 | 发布内容到云端 |
| 云端服务 | 保存内容版本、设备状态与身份信息 | 在设备醒来时返回最新状态和图片 URL |
| ESP32-C3 | 唤醒、查询、下载校验、刷新屏幕 | 只在必要时联网工作 |
| 7.3" E6 电子纸 | 呈现六色会议牌内容 | 断电后继续保留画面 |
### 工作流程（核心）

![设备唤醒与更新流程](docs/images/wake-cycle.svg)
...
- 云端没有常驻连接可用于实时在线判断：最后上报后的 5 分钟活动窗口显示“在线”；之后显示“睡眠”。第一次预计自动唤醒再加 5 分钟上报宽限后仍无状态，即显示“离线”。例如 12 小时周期会在最后上报约 12 小时 5 分钟后判定离线。
